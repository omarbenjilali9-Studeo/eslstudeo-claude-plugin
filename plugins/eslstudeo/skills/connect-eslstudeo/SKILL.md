...
- ESLStudeo labels each tool as reading or changing, so Claude's **Tool permissions** page groups
  ...
  `replace_text`, `end_class`, `manage_learner`, `remove_placement_section`, `delete_placement_test`, `set_open_placement_link` and `decide_placement`,
